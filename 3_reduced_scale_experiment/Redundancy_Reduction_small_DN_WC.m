function Redundancy_Reduction_small_DN_WC(param,num,name)
...
% 
% elseif caso == 2 
% %%% 2. REDUCED DEPENDENCE WITH LUMINANCE (because of gamma=1)
% g1= 1;
% L = [1 1 1;1 0 -1;-0.5 1 -0.5];
% L = L./repmat(sqrt(sum(L.^2,2)),1,3);
% S = diag([0.8 1 0.2]);
...
% 
% elseif caso == 3
% %%% 3. INCREASED SENSITIVITY (AND EFFICIENCY) (because of increased linear gain "CSF")
% g1= 0.7;
% L = [1 1 1;1 0 -1;-0.5 1 -0.5];
% L = L./repmat(sqrt(sum(L.^2,2)),1,3);
% S = diag([0.9 1 0.7]);
...
% 
% elseif caso == 4
% %%% 4. REDUCED SENSITIVITY (AND EFFICIENCY) (because of increased semisaturation "b")
% g1= 0.7;
% L = [1 1 1;1 0 -1;-0.5 1 -0.5];
% L = L./repmat(sqrt(sum(L.^2,2)),1,3);
% S = diag([0.8 1 0.2]);
...
% 
% else
% %%% 5. INCREASED EFFICIENCY? (because of wider kernel "H")
% g1= 0.7;
% L = [1 1 1;1 0 -1;-0.5 1 -0.5];
% L = L./repmat(sqrt(sum(L.^2,2)),1,3);
% S = diag([0.8 1 0.2]);
% H = [1 0.3 0.1;0.1 1 0.3;0.05 0.3 1];  % Wilson-Cowan Kernel ;-)
% H = H./repmat(sum(H,2),1,3);
% % H = eye(3);
% g = 1;
% b = b0/30;
...
% im = [im1 im2 im3]/256;
% load images_80_free
% im = [im;ima1 ima2 ima3;ima4 ima5 ima6];
% im = [im im';rot90(im,2) rot90(im,3)];
% im = im + 0.01*randn(size(im));
% x = im2col(im,[1 3],'sliding');
...
% for i=1:19
%     i
%     load(['im_color_',num2str(i)]);
%     im = double(Im(:,:,2))/255 + 0.001*randn(256,256);
%     im = (im - min(im(:)))/(max(im(:)) - min(im(:)));
%     x = im2col(im,[1 3],'sliding');
%     rand_indices=randperm(length(x(1,:)));
%     xx = [xx x(:,rand_indices(1:num))]; 
%     x = im2col(im',[1 3],'sliding');
%     rand_indices=randperm(length(x(1,:)));
%     xx = [xx x(:,rand_indices(1:num))];
%     im1 = im(1:2:end,1:2:end);
%     im2 = im(2:2:end,2:2:end);
%     im = [im1 im2;im1' im2'];
%     x = im2col(im,[1 3],'sliding');
%     rand_indices=randperm(length(x(1,:)));
%     xx = [xx x(:,rand_indices(1:num))];
...
%     rand_indices=randperm(length(x(1,:)));
%     xx = [xx x(:,rand_indices(1:num))];
%     figure(100),colormap gray,imagesc(im),pause(1)
% end
% x = xx;clear xx
...
contrasts = linspace(0,1.1,17);
luminances = linspace(0,2.5,16);
    ...
    for m = 1:length(contrasts)
        ...
        if H(l,m) > 100
           ...
           for jj=1:length(xx(1,:))
                ...
                J(:,:,jj)=(J2.sx)*(J1.sx);
                detJ(jj) = det(squeeze(J(:,:,jj)));
                volJND(jj) = det( squeeze(J(:,:,jj))'*squeeze(J(:,:,jj)) );
                % ---------------------------- integracion WC
                % jj
                ...
                JJ(:,:,jj) = Jo*L2*(J1o.sx);
                detJwc(jj) = det(squeeze(JJ(:,:,jj)));
                volJNDwc(jj) = det( squeeze(JJ(:,:,jj))'*squeeze(JJ(:,:,jj)) );
