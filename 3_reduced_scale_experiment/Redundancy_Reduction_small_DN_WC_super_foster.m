function Redundancy_Reduction_small_DN_WC_super_foster(param,Nfast,name,integrate)
...
%% FOSTER 3D AFTER VON KRIES AND NORMALIZED BY THE 0.975 OF CUMSUM
load(['/media/disk/vista/Papers/2019_Information_Flow_Wilson_Cowan/3D_example_jesus/data_foster_3d'])

x = samples_foster_y1;
clear samples_foster_y1
...
contrasts = linspace(0,1.1,12);
luminances = linspace(0,1.1,12);
...
for l = 1:length(luminances)
    for m = 1:length(contrasts)
        ...
        if H(l,m) > Nfast
           ...
           xx = samples(l,m).x;
           rand_indices=randperm(length(xx(1,:)));
           xx = real(xx(:,rand_indices(1:Nfast)));
           ...
           for jj=1:length(xx(1,:))
                [y1(:,jj),x1,y2(:,jj),x2(:,jj),J1,J2] = stabilized_DN_param(xx(:,jj),param,1);
                J(:,:,jj)=(J2.sx)*(J1.sx);
...
for l = 1:length(luminances)
    for m = 1:length(contrasts)
        if H(l,m) > 200
           samples_transform_0 = [samples_transform_0 samples_transf_0(l,m).y1]; 
...
for l = 1:length(luminances)
    for m = 1:length(contrasts)
        if H(l,m) > Nfast
           ...
           rand_indices=randperm(length(xx2(1,:)));
           xx2 = real(xx2(:,rand_indices(1:Nfast)));
